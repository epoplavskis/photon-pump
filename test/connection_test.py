...
async def test_subscribe_to_all(event_loop):
    async with connect(username="admin", password="changeit", loop=event_loop) as conn:
        ...
        # task = event_loop.create_task(connection_break(conn, subscription))
        # try:
        #     event_loop.run_until_complete(task)
        # except asyncio.CancelledError:
        #     pass
        
        # event_loop.call_later(1, new_event, conn)
        # event_loop.call_later(3, stop, subscription)
        event_ids_list = []
...
#        assert event_id_2 in event_ids_list
        #     event = await subscription.events.anext()

#        event = await subscription.events.anext()
#        assert event.received_event.id == event_id_2
